...
Room URL: https://tryhackme.com/room/mayhemroom
...
This confirms that the binary (`notepad.exe`) is a Trojan malware. 
...
In stream 5, this is Command and Control (C2) and exfiltration traffic generated by the Trojan binary. The infected host is sending repeated POSTs request with `Content-Type: */*` which mean he is accepting any content type from the server (`10.0.2.37:80`), the attacker response with raw data. 

I tried to save the steam and do manual searching, but this wasn't the right approach. After some research, I used the pcap hash and searched for the file in VirusTotal

<img width="1915" height="941" alt="Screenshot 2025-07-12 201834" src="https://github.com/user-attachments/assets/69f5edc1-e41b-414b-beca-7db33b034e8b" />

Upon reading the community comments, this looks like an open-source post-exploitation framework ->  https://github.com/HavocFramework. 

## Obtaining the encryption keys from packet capture

Referred to this article -> https://www.immersivelabs.com/resources/blog/havoc-c2-framework-a-defensive-operators-guide, it shows how to detect and find the encryption keys and how to decrypt the traffic
